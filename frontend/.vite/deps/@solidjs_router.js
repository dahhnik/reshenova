...
function createRouterContext(integration, branches, getContext, options = {}) {
  ...
  function preloadRoute(url, options2 = {}) {
    ...
    for (let match in matches2) {
      ...
      inLoadFn = true;
      options2.preloadData && load && runWithOwner(getContext(), () => load({
        params: params2,
        location: {
...
function StaticRouter(props) {
  let e;
  const url = props.url || (e = voidFn()) && getPath(e.request.url) || "";
  const obj = {
...
function cache(fn, name) {
  ...
  const cachedFn = (...args) => {
    ...
    if (isServer) {
      ...
      if (e) {
        ...
        if (dataOnly2) {
          ...
          if (Array.isArray(dataOnly2) && !dataOnly2.includes(key)) {
            data[key] = void 0;
    ...
    if (cached && cached[0] && (isServer || intent2 === "native" || cached[3].count || Date.now() - cached[0] < PRELOAD_TIMEOUT)) {
      ...
      inLoadFn2 && "then" in res2 && res2.catch(() => {
      });
      return res2;
    }
    ...
    inLoadFn2 && "then" in res && res.catch(() => {
    });
